...
2. 取り込み（テキストファイルから）：
   ...
   - DOI 付き論文をさらに整えたい場合は、取り込み後に「業績DB」メニューの「選択行を補完」で補強できます（DOI/タイトル検索/ISBN を自動で使い分け）。
...
| 重複・欠損を点検する | スプレッドシート | 「業績DB」→「★ まとめて点検」で全タブを再判定＋色付け（**欠損=橙／未確認=黄／重複=赤太字**）し、`点検レポート` シートに問題行を一覧。重複は残す方を `確認済`・重複側を削除、欠損は不足項目を補う。 |
| DOI/ISBN から情報を補う | スプレッドシート | 行を選択 →「業績DB」→「選択行を補完」（論文=DOI/タイトル検索→CrossRef、書籍=ISBN→OpenLibrary）。 |
