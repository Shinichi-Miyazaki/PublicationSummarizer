...
- **タイトルの整形**：タイトルを囲う括弧・引用符（`「」『』""()` 等）は表示時に自動で外す（`TNF-α (review)` のような途中の括弧は保持）。
- **著者の表示**：表示人数の上限（0＝全員）と省略語（言語自動「ほか」/「et al.」・「ほかN名」も可）を種別ごとに設定。
  サイドバーで著者を選ぶと、出力中の**その名前を自動で太字**にする。省略時も**選択著者は必ず残す**（既定）。
  **名（ファーストネーム）のイニシャル化**も可能：名簿で照合できた著者を「姓＋名イニシャル」へ整形する（形式は `Yamada T` /「`T. Yamada`」を選択。照合できない外部共著者は元の表記のまま）。
- **出力**：リッチ表示（書式保持コピー用）＋プレーンテキスト（折りたたみ）。**該当0件の種別・全0件の著者は自動で非表示**。
  複数種別を表示中は末尾に**「すべてまとめてコピー」**（全種別を1つに結合）も表示。
...
scripts/llm_parse.py            任意: OpenAI API で貼り付けを構造化抽出（--llm 用・未導入でも基本機能は動作）
scripts/forms/publication_form.gs  Apps Script: 1フォーム生成＋送信を Canonical へ自動取込（英日両入力・DOI/ISBN補完・タイトル→DOI検索・一括貼り付けの任意LLM構造化＋区分の選択・重複フラグ）
...
著者の表示はプリセット単位でも既定値を持てる（いずれも画面から上書き可）:
`author_max`（表示人数・0=全員）/ `author_etal`（省略語・空=言語自動）/ `author_etal_count`（「ほかN名」）/
`author_keep_highlighted`（選択著者を必ず残す）/ `author_emphasis`（`bold`=選択著者を太字 / `none`）/
`author_initials`（`true`=名をイニシャル化）/ `author_initials_format`（`last_first`=「Yamada T」/ `first_last`=「T. Yamada」）。
