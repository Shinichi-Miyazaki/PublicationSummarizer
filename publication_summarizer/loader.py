"""Google Spreadsheet（リンク共有・閲覧可）から業績を取得し正規化する。

認証不要。`/export?format=xlsx` で全シートを1リクエスト取得し、各シートを
schema.py の列インデックス仕様に従って共通スキーマの DataFrame に正規化する。
"""
...
META_COLUMNS = ["type", "label", "no", "date", "fiscal_year", "authors_raw"]
...
def _is_real_row(no_value, marker_value) -> bool:
    """実データ行か判定する（Example 行・空行・見出し行を除外）。

    実データは A列(No) が数値。Example 行は A列="Example" / B列(marker)="例"。
    """
...
def _cell(row: pd.Series, idx: int) -> str:
    """指定列インデックスの値を文字列で返す（NaN は空文字）。"""
    if idx >= len(row):
        return ""
    val = row.iloc[idx]
...
def _normalize_sheet(raw: pd.DataFrame, spec: SheetSpec) -> list[dict]:
    """1シートの生 DataFrame を共通スキーマの dict 行リストに変換。"""
    records: list[dict] = []
    marker_idx = 1  # B列 = Example マーカー列
    no_idx = spec.cols.get("no", 0)

    for _, row in raw.iterrows():
        no_val = row.iloc[no_idx] if no_idx < len(row) else None
        marker_val = row.iloc[marker_idx] if marker_idx < len(row) else None
        if not _is_real_row(no_val, marker_val):
            continue

        rec: dict = {"type": spec.rtype, "label": spec.label}
        for logical, idx in spec.cols.items():
            rec[logical] = _cell(row, idx)

        # 連番だけ振られた空テンプレート行を除外（著者・主タイトル・日付がいずれも空なら捨てる）。
